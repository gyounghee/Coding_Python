...
## 다른 사람 풀이 해석 후 내 방식대로 변형
import re

def solution(str1, str2):
    str1 = [str1[i:i+2].lower() for i in range(0, len(str1)-1) if re.findall('[a-zA-Z]+[a-zA-Z]', str1[i:i+2])]
    str2 = [str2[i:i+2].lower() for i in range(0, len(str2)-1) if re.findall('[a-zA-Z]+[a-zA-Z]', str2[i:i+2])]
